...
for filename in os.listdir("actual_train"):
    for ws_use in ws_range:
        ...
        model_name = "GRU_Att"
        if not os.path.isdir("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/models/" + model_name):
            os.makedirs("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/models/" + model_name)

        if not os.path.isdir("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/train/" + model_name):
            os.makedirs("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/train/" + model_name)
    
        if not os.path.isdir("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/test/" + model_name):
            os.makedirs("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/test/" + model_name)
    
        if not os.path.isdir("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/val/" + model_name):
            os.makedirs("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/val/" + model_name)
        
        save_object("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/models/" + model_name + "/" + varname + "_" + model_name + "_ws_" + str(ws_use) + "_train_losses", train_losses)

        save_object("train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/models/" + model_name + "/" + varname + "_" + model_name + "_ws_" + str(ws_use) + "_val_losses", val_losses)

        model.seq2seq.eval()
        torch.save(model.seq2seq.state_dict(), "train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/models/" + model_name + "/" + varname + "_" + model_name + "_ws_" + str(ws_use) + ".pth")
        ...
        with torch.no_grad():
            ...
            print_predictions(y_train_all, predict_train_all, "train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/train/" + model_name + "/" + varname + "_" + model_name + "_ws_" + str(ws_use) + "_train.csv") 
            ...
            print_predictions(y_val_all, predict_val_all, "train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/val/" + model_name + "/" + varname + "_" + model_name + "_ws_" + str(ws_use) + "_val.csv")     
            ...
            print_predictions(y_test_all, predict_test_all, "train_attention" + str(ws_use - min(ws_range) + 5 + 3 * len(ws_range)) + "/" + varname + "/predictions/test/" + model_name + "/" + varname + "_" + model_name + "_ws_" + str(ws_use) + "_test.csv")