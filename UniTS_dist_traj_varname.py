...
if os.path.isfile("UniTS_final_result_varname/distance_predicted_new"):
    distance_predicted_new = load_object("UniTS_final_result_varname/distance_predicted_new")
    ...
    for model_name in predicted_long:
        ...
        for ws_use in predicted_long[model_name]:
            ...
            for dist_name in predicted_long[model_name][ws_use]:
                ...
                if "UniTS_varname" not in dicti_to_print[dist_name]:
                    dicti_to_print[dist_name]["UniTS_varname"] = dict()
                    
                if str(ws_use) not in dicti_to_print[dist_name]["UniTS_varname"]:
                    dicti_to_print[dist_name]["UniTS_varname"][str(ws_use)] = dict()

                dicti_to_print[dist_name]["UniTS_varname"][str(ws_use)]["R2_wt"] = r2_pred_wt
                dicti_to_print[dist_name]["UniTS_varname"][str(ws_use)]["MAE_wt"] = mae_pred_wt
                dicti_to_print[dist_name]["UniTS_varname"][str(ws_use)]["RMSE_wt"] = rmse_pred_wt
                dicti_to_print[dist_name]["UniTS_varname"][str(ws_use)]["Euclid"] = np.average(vals_avg)
                dicti_to_print[dist_name]["UniTS_varname"][str(ws_use)]["R2"] = r2_pred
                dicti_to_print[dist_name]["UniTS_varname"][str(ws_use)]["MAE"] = mae_pred
                dicti_to_print[dist_name]["UniTS_varname"][str(ws_use)]["RMSE"] = rmse_pred

save_object("UniTS_final_result_varname/distance_predicted_new", distance_predicted_new)
...
list_ws = sorted([int(x) for x in dicti_to_print["long no abs"]["UniTS_varname"]])
