...
for ws_use in ws_range:
    ...
    for filename in os.listdir("actual_train"):
        ...
        if varname != "time":
            file_pd_transformed_pred = np.array(load_object("../UniTS/results/all_no_time_" + str(ws_use) + "_test/preds_transformed_" + varname))
            file_pd_transformed_true = np.array(load_object("../UniTS/results/all_no_time_" + str(ws_use) + "_test/trues_transformed_" + varname))
            file_pd_transformed_xs = np.array(load_object("../UniTS/results/all_no_time_" + str(ws_use) + "_test/xs_transformed_" + varname))
        else:
        ...
        if not os.path.isdir("UniTS_final_res_no_time/" + str(ws_use) + "/"):
            os.makedirs("UniTS_final_res_no_time/" + str(ws_use) + "/")

        df_new.to_csv("UniTS_final_res_no_time/" + str(ws_use) + "/" + varname + ".csv", index = False) 
