...
for varname in os.listdir("train_pytorch"):
    ...
    for model_name in os.listdir("train_pytorch/" + varname + "/predictions/val/"):
        ...
        for filename in os.listdir("train_pytorch/" + varname + "/predictions/val/" + model_name):
            ...
            train_data = pd.read_csv("train_pytorch/" + varname + "/predictions/train/" + model_name + "/" + filename.replace("val", "train"), sep = ";", index_col = False)
            
            test_data = pd.read_csv("train_pytorch/" + varname + "/predictions/test/" + model_name + "/" + filename.replace("val", "test"), sep = ";", index_col = False)
        ...
        final_val_data = pd.read_csv("final_train_pytorch/" + varname + "/predictions/val/" + model_name + "/" + varname + "_" + model_name + "_ws_" + str(ws_use) + "_hidden_" + str(hidden_use) + "_val.csv", sep = ";", index_col = False)
            
        final_train_data = pd.read_csv("final_train_pytorch/" + varname + "/predictions/train/" + model_name + "/" + varname + "_" + model_name + "_ws_" + str(ws_use) + "_hidden_" + str(hidden_use) + "_train.csv", sep = ";", index_col = False)
        ...
        if is_a_nan:
            final_val_R2.append(1000000)
        else: 
            final_val_R2.append(r2_score(final_val_data["actual"], final_val_data["predicted"]))
        ...
        if is_a_nan:
            final_train_R2.append(1000000)
        else: 
            final_train_R2.append(r2_score(final_train_data["actual"], final_train_data["predicted"]))
