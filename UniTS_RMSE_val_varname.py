...
for varname in os.listdir("final_train_pytorch"):
    ...
    for mini_ix_val in range(len(final_test_RMSE)):
        ...
        if "UniTS_varname" not in dicti_to_print[varname]:
            dicti_to_print[varname]["UniTS_varname"] = dict()
        dicti_to_print[varname]["UniTS_varname"][str(ws_use)] = dict()
        dicti_to_print[varname]["UniTS_varname"][str(ws_use)]["NRMSE"] = final_test_NRMSE[mini_ix_val] 
        dicti_to_print[varname]["UniTS_varname"][str(ws_use)]["RMSE"] = final_test_RMSE[mini_ix_val] 
        dicti_to_print[varname]["UniTS_varname"][str(ws_use)]["R2"] = final_test_R2[mini_ix_val]  
        dicti_to_print[varname]["UniTS_varname"][str(ws_use)]["MAE"] = final_test_MAE[mini_ix_val]  
...
list_ws = sorted([int(x) for x in dicti_to_print["speed"]["UniTS_varname"]])
